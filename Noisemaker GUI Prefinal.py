...
class NoiseMaker:
    ...
    def create_widgets(self):
        ...
        self.bg_image = Image.open("C:/Users/Amit/Documents/music-player-code/background_image.jpg.jpg")
        self.bg_image = self.bg_image.filter(ImageFilter.GaussianBlur(radius=0))
        self.bg_photo = ImageTk.PhotoImage(self.bg_image)

        # Get the dimensions of the background image
        bg_width, bg_height = self.bg_image.size

        # Set window size according to the background image
        ...
        self.scroll_bar = Scrollbar(self.listbox_frame, orient=VERTICAL, command=self.playlist.yview, bg="Gray16", troughcolor="#1E1E1E")
        self.scroll_bar.pack(side=RIGHT, fill=Y)

        self.playlist.config(yscrollcommand=self.scroll_bar.set)

        # SongFrame Labels
        Label(self.song_frame, text='CURRENTLY PLAYING:', bg="#b5e4e3", fg="black", font=('Helvetica', 10, 'bold')).pack(pady=5)

        self.song_lbl = Label(self.song_frame, textvariable=self.current_song, bg="Gray16", fg="white", font=('Helvetica', 12))
        ...
        self.volume_slider = Scale(self.root, from_=0, to=100, orient=HORIZONTAL, label="Volume", bg='Gray16', fg="white",
        ...
        Label(self.root, textvariable=self.song_status, bg='Lavender', fg="Black", font=('Helvetica', 9), justify=LEFT).grid(row=3, column=0, columnspan=2, sticky="ew")
